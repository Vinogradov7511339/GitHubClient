...
private extension RepositoryAdapterImpl {
    ...
    func viewModel(_ indexPath: IndexPath) -> Any? {
        ...
        case .code:
            return codeViewModel(for: indexPath.row, repository: repository.repository)
        case .info:
            return infoViewModel(for: indexPath.row, repository: repository.repository)
        case .readMe:
            return ReadMeCellViewModel(mdText: repository.mdText)
        }
    }

    func codeViewModel(for row: Int, repository: Repository) -> Any? {
        switch row {
        case 0:
    ...
    func infoViewModel(for row: Int, repository: Repository) -> Any? {
        switch row {
        case 0:
